...
public class DLX {
        ...
        try {
            ...
            while (true) {
                ...
                switch (op) {
                    ...
                    case LSHI:
                        if ((c < -31) || (c >31)) {
                            System.out.println("Illegal value " + c + " of operand c or register c!");
                            bug(1);
                        }
                        if (c < 0) {
                            R[a] = R[b] >>> -c;
                    ...
                    case ASHI:
                        if ((c < -31) || (c >31)) {
                            System.out.println("DLX.execute: Illegal value " + c + " of operand c or register c!");
                            bug(1);
                        }
                        if (c < 0) {
                            R[a] = R[b] >> -c;
                    ...
                    case CHK:
                        if (R[a] < 0) {
                            System.out.println("DLX.execute: " + PC*4 + ": R[" + a + "] == " + R[a] + " < 0");
                            bug(14);
                        }
                        else if (R[a] >= c) {
                            System.out.println("DLX.execute: " + PC*4 + ": R[" + a + "] == " + R[a] + " >= " + c);
                            bug(14);
                        }
                        break;
                    case LDW:
                    ...
                    case BEQ:
                        ...
                        if ((nextPC < 0) || (nextPC > MemSize/4)) {
                            System.out.println(4*nextPC + " is no address in memory (0.." + MemSize + ").");
                            bug(40);
                        }
                        break;
                    case BNE:
                    ...
                    case RET:
                        ...
                        if ((c < 0) || (c > MemSize)) {
                            System.out.println(c + " is no address in memory (0.." + MemSize + ").");
                            bug(49);
                        }
                        nextPC = c / 4;
                        break;
    ...
    static int F3(int op, int c) {
        if ((c < 0) || (c > MemSize)) {
            System.out.println("Operand for F3 Format is referencing non-existent memory location.");
            bug(1);
        }
        return op << 26 | c;
    }
